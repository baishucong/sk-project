	...
	function slide(slideMode){//轮播方法		
		if (isMoving==false){
			...
			if(slideMode){			//slideMode为1(true)，执行slide(1)，上一张
				...
				hidden.css({width:"1026px",height:"580px",top:"60px",left:"0px",opacity:0});
				hidden.stop(true,true).animate({width:"1026px",height:"580px",top:"20px",left:"0px",opacity:1},timer);
				curr.stop(true,true).animate({width:"1026px",height:"580px",top:"20px",left:"600px",opacity:1},timer);
				next.stop(true,true).animate({width:"1026px",height:"580px",top:"60px",left:"730px",opacity:0},timer,function(){next.find("span").css("opacity",0); isMoving = false;});
				//prev  -->  curr     prev中的图片li轮换到curr的位置      其他一次轮换
				prev.find("span").css("opacity",0);
				$(".main_banner_box li").find("p").css({"bottom":"-50px"});//所有标题隐藏
				prev.stop(true,true).animate({width:"1239px",height:"700px",opacity:1},timer,function(){
				...
				hidden.css({width:"1026px",height:"580px",top:"60px","left":"730px","opacity":0});
				hidden.stop(true,true).animate({width:"1026px",height:"580px",top:"60px",left:"600px",opacity:1},timer);							
				curr.stop(true,true).animate({width:"1026px",height:"580px",top:"60px",left:"0px",opacity:1},timer);
				//next  -->  curr     next中的图片li轮换到curr的位置      其他一次轮换
				next.find("span").css("opacity",0);
				$(".main_banner_box li").find("p").css({"bottom":"-50px"});//所有标题隐藏
				next.stop(true,true).animate({width:"1239px",height:"700px",left:"340px",top:0,opacity:1},timer,function(){
					$(this).find("p").animate({"bottom":"0px"});	//当前这张图片的标题运动出来
				});
				prev.stop(true,true).animate({width:"1026px",height:"580px",left:"0px",top:"200px",opacity:0},timer,function(){
					isMoving = false;
				}); 
				index_2++;	
			}//if else
	...
	if(len>3){
		...
		$(".btn_list span").click(function(event){
			...
			if(Math.abs(index_2-oIndex)>1&&Math.abs(len-Math.abs(index_2-oIndex))!=1){
				//统一样式
				$(".main_banner_box li").css({width:"1026px",height:"580px",left:"600px",top:"60px",opacity:0});
...
//			intervaltimer=setInterval(slide,3000);//自动轮播
		...
		$(".main_banner_box li").on("mousemove",function(){
			if($(this).css("width")=="1239px"){//鼠标移入为当前正中显示的图片li，则清除定时器
				clearInterval(intervaltimer);
				intervaltimer=null;
			}
